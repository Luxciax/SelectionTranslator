...
function actionTitle(request: ActionRequest | null): string {
  if (!request) return 'SelectionTranslator'
...
export function ResultWindow() {
  ...
  return (
    <div className={`result-shell${compact ? ' compact' : ''}${isHome ? ' home' : ''}`} onContextMenu={(event) => event.preventDefault()}>
      ...
      {isHome ? (
        <main className="home-body">
          <div className="home-mark"><Sparkles size={22} strokeWidth={1.6} /></div>
          <div className="home-status"><span className="home-status-dot" />已在运行</div>
          <h1>SelectionTranslator</h1>
          <p>选中文字后会出现浮动工具条，可直接翻译、解释、总结、润色或运行自定义动作。</p>
          <button type="button" className="home-settings" onClick={() => setSettingsOpen(true)}>
            <Settings2 size={14} strokeWidth={1.8} />
            <span>设置</span>
          </button>
          <div className="home-model">{settings.model || '尚未配置模型'}</div>
        </main>
      ) : (
      <main className="result-body">
        ...
        <section className="result-content" aria-live="polite">
          {content ? (
            ...
            <div className="result-empty"><Sparkles size={18} strokeWidth={1.6} /><span>等待结果</span></div>
          )}
        </section>
